...
//     btTransform tr(btQuaternion(x(3), x(4), x(5), x(6)), btVector3(x(0),
//     x(1), x(2))); return tr;
// }
...
PlanningWorld::PlanningWorld() { this->cons = new ContactConstraints(2); }
...
Vector6d PlanningWorld::QPVelocity(const Vector6d &v_goal,
  ...
  this->cons->ModeConstraints(envs, env_mode, this->mu_env, f_ext_o, &A_env,
                              &b_env, &G_env, &h_env);
  // std::cout << "A_env:\n" << A_env << std::endl;
  // Vector6d vv;
  // vv << -1,1,0,0,0,1;
  ...
  this->cons->ModeConstraints(mnps, mmode, this->mu_mnp, f_ext_o, &A_mnp,
                              &b_mnp, &G_mnp, &h_mnp);
...
Vector6d PlanningWorld::QPVelocity_Quasidynamic(
    const Vector6d &v_goal, const std::vector<ContactPoint> &mnps,
    const std::vector<ContactPoint> &envs, const VectorXi &env_mode,
    const Vector6d &f_ext_o, double wt, double wa, double h_time) {
  ...
  G.block(G.rows() - 12, 0, 6, 6) =
      -this->object_inertia * 1 / (h_time); // MI: object inertia
...
bool PlanningWorld::ForwardIntegration(
    const Vector7d &x_start, const Vector7d &x_goal, const VectorXd &mnp_config,
  ...
  //     std::cout << "Environment contacts and modes are not consistent" <<
  //     std::endl; if(envs_.size()==2){
  ...
  for (counter = 0; counter < max_counter; counter++) {
    ...
    Vector6d v_b;
    if (ifquasidynamic) {
      v_b = QPVelocity_Quasidynamic(v_star, mnps, envs, env_mode, f_ext_o, wt,
                                    wa, 1);
    } else {
      v_b = QPVelocity(v_star, mnps, envs, env_mode, f_ext_o, wt, wa);
    }

    // Vector6d v_quasidynamic = QPVelocity_Quasidynamic(v_star, mnps, envs,
    // env_mode, f_ext_o, wt, wa, 1); Vector6d v_quasistatic =
    // QPVelocity(v_star, mnps, envs, env_mode, f_ext_o, wt, wa); double d2 =
    // (v_quasidynamic - v_quasistatic).norm();
    ...
    // velocity correction
    int pre_env_size = env_mode.size() / 3;
...
Vector6d PlanningWorld::EnvironmentConstrainedVelocity(
  ...
  this->cons->ModeConstraints(envs, env_mode, this->mu_env, Vector6d::Zero(),
                              &A_env, &b_env, &G_env, &h_env);
