...
Vector7d steer_config_directional(Vector7d x_near, Vector7d x_rand, Vector6d v,
...
RRTPlanner::RRTPlanner(PlanningWorld *pw_) { this->pw = pw_; }

void RRTPlanner::Initialize(Vector3d q_lb, Vector3d q_ub, Vector7d q_start,
                            Vector6d f_w, std::vector<ContactPoint> &surface,
                            double angle_weight, double translation_weight,
                            double eps_trans, double eps_angle) {
  // angle_weight and translation_weight: heuristics of how important the
  // dimensions are.
...
bool RRTPlanner::randomRelocateFingers(const VectorXd &mnp_config, Vector7d x,
                                       ...
                                       const VectorXi &env_mode, Vector6d f_o,
                                       VectorXd &new_config) {
  ...
  for (int k_sample = 0; k_sample < samples; k_sample++) {
    ...
    bool isresample = this->pw->world->getRobot()->resampleFingers(
        n_on, mnp_config, x, this->object_surface, new_mnp_config,
        &remain_mnps);
    ...
    this->pw->world->getRobot()->Fingertips2PointContacts(remain_mnps,
                                                          &remain_grasp_mnps);
    bool isremainbalance =
        this->pw->isForceBalance(remain_grasp_mnps, envs, relocate_mode, f_o);

    std::vector<ContactPoint> grasp_mnps;
    std::vector<ContactPoint> mnp_fingertips;
    this->pw->world->getRobot()->getFingertipsOnObject(new_mnp_config, x,
                                                       &mnp_fingertips);
    this->pw->world->getRobot()->Fingertips2PointContacts(mnp_fingertips,
                                                          &grasp_mnps);
...
bool RRTPlanner::randomRelocateFingers_Quasidynamic(
    const VectorXd &mnp_config, Vector7d x, Vector6d v,
    const std::vector<ContactPoint> &envs, const VectorXi &env_mode,
    Vector6d f_o, VectorXd &new_config) {
  // std::vector<ContactPoint> fingertips;
  // this->pw->world->getRobot()->getFingertips(mnp_config, &fingertips);
  this->pw->world->updateObjectPose(x);
  VectorXd new_mnp_config(0);
  ...
  for (int k_sample = 0; k_sample < samples; k_sample++) {
    ...
    Vector6d v_dyn_remain = this->pw->QPVelocity_Quasidynamic(
        v, remain_grasp_mnps, envs, env_mode, f_o, T->translation_weight,
        T->angle_weight, 1);
    ...
    Vector6d v_dyn = this->pw->QPVelocity_Quasidynamic(
        v, grasp_mnps, envs, env_mode, f_o, T->translation_weight,
        T->angle_weight, 1);
...
void RRTPlanner::Extend_QPCC(int near_idx, Vector7d x_rand_) {
  ...
  // steer goal
  Vector7d x_rand = steer_config(T->nodes[near_idx].config, x_rand_,
                                 this->eps_trans, this->eps_angle);
  ...
  if (T->nodes[near_idx].modes.size() == 0) {
    ...
    this->pw->world->getObjectContacts(&(T->nodes[near_idx].envs),
                                       T->nodes[near_idx].config);
  ...
  for (const auto &cs_mode : T->nodes[near_idx].modes) {
    ...
    std::vector<VectorXi> mode_to_extend;
    {
      ...
      Vector6d v = this->pw->EnvironmentConstrainedVelocity(
          v_star, T->nodes[near_idx].envs, all_sticking_mode);
      // std::cout << "mode: " << all_sticking_mode.transpose() <<  " velocity:
      // " << v.transpose() << std::endl;
      if (dist_vel(v, v_star, T->translation_weight, T->angle_weight) <
          d_zero) {
    ...
    for (const auto &mode : modes) {
      Vector6d v = this->pw->EnvironmentConstrainedVelocity(
          v_star, T->nodes[near_idx].envs, mode);
    ...
    for (const auto &mode : mode_to_extend) {
      ...
      if (T->nodes[near_idx].manipulator_config.size() == 0) {
        ...
        if (ifmnpcollide) {
          ...
          this->pw->world->getRobot()->getFingertipsOnObject(
              T->nodes[near_idx].manipulator_config, T->nodes[near_idx].config,
              &mnp_fingertips);
          this->pw->world->getRobot()->Fingertips2PointContacts(mnp_fingertips,
                                                                &grasp_mnps);

          bool isbalance = this->pw->isForceBalance(
              grasp_mnps, T->nodes[near_idx].envs, mode, f_o);
      ...
      if (ifChangeFingertips) {
        ...
        ifchanged = this->randomRelocateFingers(
            T->nodes[near_idx].manipulator_config, T->nodes[near_idx].config,
      ...
      ifmanipulatorcollide = this->pw->ForwardIntegration(
          T->nodes[near_idx].config, x_rand, mnp_config,
          T->nodes[near_idx].envs, mode, this->f_w, T->translation_weight,
          T->angle_weight, &path);
...
void RRTPlanner::Extend_QPCC_Quasidynamic(int near_idx, Vector7d x_rand_) {
  ...
  for (const auto &cs_mode : T->nodes[near_idx].modes) {
    ...
    for (const auto &mode : mode_to_extend) {
      ...
      Vector6d v_mode = this->pw->EnvironmentConstrainedVelocity(
          v_star, T->nodes[near_idx].envs, mode);
      // check if we can try direct forward integration
      bool ifChangeFingertips = false;
      ...
      // } else
      {
        ...
        if (ifmnpcollide) {
          ...
          Vector6d v_dyn = this->pw->QPVelocity_Quasidynamic(
              v_mode, grasp_mnps, T->nodes[near_idx].envs, mode, f_o,
              T->translation_weight, T->angle_weight, 1);

          // Vector6d v_static = this->pw->QPVelocity(v_star, fingertips,
          // T->nodes[near_idx].envs,
          //     mode, f_o, T->translation_weight, T->angle_weight);

          // bool isforcebalance = this->pw->isForceBalance(fingertips,
          // T->nodes[near_idx].envs,
          //     mode, f_o);
          ...
          if (v_dyn.norm() < 1e-4) {
            ...
            isdynamicfeasible = v_mode.dot(v_dyn * (1 / v_mode.norm())) >
                                0.2 * v_mode.dot(v_mode * (1 / v_mode.norm()));
          }
      ...
      if (ifChangeFingertips) {
        ...
        ifchanged = this->randomRelocateFingers_Quasidynamic(
            T->nodes[near_idx].manipulator_config, T->nodes[near_idx].config,
            v_mode, T->nodes[near_idx].envs, mode, f_o, mnp_config);
      ...
      ifmanipulatorcollide = this->pw->ForwardIntegration(
          T->nodes[near_idx].config, x_rand, mnp_config,
          T->nodes[near_idx].envs, mode, this->f_w, T->translation_weight,
          T->angle_weight, &path, ifquasidynamic);
...
void RRTPlanner::Search(const RRTPlannerOptions &opts, Vector7d x_goal,
                        double goal_thr, std::vector<int> *node_path,
                        bool &ifsuccess, double &time_cpu_s,
                        bool ifquasidynamic) {
  ...
  for (int kk = 0; kk < this->options->max_samples; kk++) {
    ...
    // std::cout << "x_rand:" << x_rand.transpose() << ", near idx: "<< near_idx
    // << std::endl;
    ...
    */
    ...
    if (dd <= goal_thr) {
      ...
      std::cout << T->nodes[goal_near_idx].manipulator_config.transpose()
                << std::endl;
    ...
    if (kk % 100 == 0) {
      std::cout << "Iteration: " << kk << " Nodes Expanded: " << T->nodes.size()
                << std::endl;
    }
  }
  ...
  if (goal_idx != -1) {
    ...
    std::cout << "GOAL NOT REACHED. Dist: "
              << T->dist(T->nodes[goal_idx].config, x_goal)
              << ". Closest config: " << T->nodes[goal_idx].config.transpose()
              << std::endl;
  }

  // std::cout << "Nearest Mode: " <<
  // T->edges[T->nodes[goal_idx].edge].mode.transpose() << std::endl;
...
double grasp_measure(VectorXd mnp_config, const std::vector<ContactPoint> &envs,
                     double grasp_measure_charac_length) {

  // measure the distance between the centroid of grasp and object com
  int n_mnps = mnp_config.size() / 6;

  Vector3d centroid;
  centroid.setZero();
  int n_pts = n_mnps;

  for (int k = 0; k < n_mnps; k++) {
    centroid += mnp_config.segment(k * 6, 3);
  }
  for (auto pt : envs) {
    centroid += pt.p;
    n_pts += 1;
  ...
  double d = centroid.norm() / grasp_measure_charac_length;

  return d;
}

void RRTPlanner::printResults(const std::vector<int> &node_path, bool success,
                              double time_cpu_s,
                              double grasp_measure_charac_length) {
  // visualization
  std::vector<Vector7d> object_poses;
  ...
  // Number of Environment Changes
  int number_env_changes = 0;
  for (int i = 0; i < node_path.size() - 1; ++i) {
    if (T->nodes[node_path[i]].envs.size() !=
        T->nodes[node_path[i + 1]].envs.size()) {
      number_env_changes++;
    }
  }
  ...
  for (auto &k : node_path) {
    ...
    double grasp_d =
        grasp_measure(T->nodes[k].manipulator_config, T->nodes[k].envs,
                      grasp_measure_charac_length);
    avg_grasp_d += grasp_d;
  }
  avg_grasp_d /= double(node_path.size() - 1);
  ...
  double travel_dist_ratio =
      travel_dist / T->dist(object_poses[0], object_poses.back());
  ...
  std::cout << "Avg grasp measure: " << avg_grasp_d << std::endl;
  std::cout << "Number of environment changes: " << number_env_changes
            << std::endl;

  std::cout << time_cpu_s << "\t" << success << "\t" << T->nodes.size() << "\t"
            << node_path.size() << "\t" << travel_dist_ratio << "\t"
            << finger_relocations << avg_grasp_d << number_env_changes<< std::endl;
}